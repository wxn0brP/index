...
const fuse = new Fuse(allRepos, {
    keys: ["name", "description", "meta.summary", "topics"],
    includeScore: true,
    threshold: 0.4,
});
...
function repoAgeTime(repo: RepoData) {
    return repo.created_at ? new Date(repo.created_at).getTime() : Number.POSITIVE_INFINITY;
}

function populateFilters() {
    addOptions(langFilter, uniqueSorted(allRepos.map(repo => repo.language)));
    addOptions(tagFilter, uniqueSorted(allRepos.flatMap(repo => getProjectTags(repo))));
    addOptions(statusFilter, uniqueSorted(allRepos.map(repo => getProjectStatus(repo))));
}
...
function applyFilters() {
    ...
    const searchMatches = query
        ? new Set((fuse.search(query) as { item: RepoData }[]).map(result => result.item))
        : null;
    ...
    const filteredData = all
        .map(([category, repos]) => {
            ...
            return [category, sortedRepos(categoryRepos, sort)];
        })
...
for (const control of [searchInput, langFilter, tagFilter, statusFilter, sortSelect]) {
...
document.addEventListener("click", (event) => {
    const button = (event.target as HTMLElement).closest<HTMLButtonElement>("[data-filter][data-value]");
    if (!button) return;
