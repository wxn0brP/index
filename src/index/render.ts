...
function renderProjectCard(repo: RepoData) {
	const lang = repo.language
		? langMap[repo.language] || repo.language.toLowerCase()
		: "";
	const icon = `<img class="lang-icon" src="${link(lang || "markdown")}" alt="">`;
	const description =
		repo.meta?.summary || repo.description || "No description provided.";
	const tags = getProjectTags(repo);
	const status = getProjectStatus(repo);
	const titleAttrs = repoCreatedLabel(repo)
		? ` title="${repoCreatedLabel(repo)}"`
		: "";
